...
var isRetryableError = function isRetryableError(err) {
  if (!err) {
    return false;
  } else if (!err.error) {
    return false;
...
git.diffFile = function (repoPath, filename, sha1, ignoreWhiteSpace) {
...
git.discardChangesInFile = function (repoPath, filename) {
  return git.status(repoPath, filename).then(function (status) {
    ...
    var fullPath = path.join(repoPath, filename);
    ...
    if (fileStatus.staged) {
      ...
      return fs.unlinkAsync(fullPath).catch(function (err) {
        throw { command: 'unlink', error: err };
      });
    }

    return fs.isExists(fullPath).then(function (isExists) {
      return isExists ? fs.lstatAsync(fullPath).then(function (stats) {
        return stats.isDirectory();
      }) : false;
    }).then(function (isSubrepoChange) {
      if (isSubrepoChange) {
        return git(['submodule', 'sync'], repoPath).then(function () {
          return git(['submodule', 'update', '--init', '-f', '--recursive', filename], repoPath);
        });
      } else {
...
git.log = function (path, limit, skip, maxSearchIteration) {
  return git(['log', '--decorate=full', '--date=default', '--pretty=fuller', '--branches', '--tags', '--remotes', '--parents', '--no-notes', '--numstat', '--date-order', '--max-count=' + limit, '--skip=' + skip], path).then(gitParser.parseGitLog).then(function (log) {
    log = log ? log : [];
